...
define(['js/Constants',
  ...
  ModelDecoratorDiagramDesignerWidget.prototype._autocompleteHelper = function (cb) {
    var self = this;
    var client = self._control._client;
    var nodeObj = client.getNode(self._metaInfo[CONSTANTS.GME_ID]);

    if (self._territoryId) {
      client.removeUI(self._territoryId);
    }
    self._territoryId = client.addUI(self, function (event) {
      var data = {};
      ...
      for (var i = childrenIds.length - 1; i >= 0; i--) {
        ...
        if (isDescendantOfComponent(child)) {
          ...
          for (var j = 0; j < grandChildrenIds.length; j++) {
            ...
            var interface_pointer = grandchild.getPointer('interface');
            if (interface_pointer && interface_pointer.to) {
    ...
    self._selfPatterns[self._metaInfo[CONSTANTS.GME_ID]] = { children: 2 };
    client.updateTerritory(self._territoryId, self._selfPatterns);
